...
const EssentialDownload = () => {
  ...
  const downloadList = [
    { title: "Conference Poster", file: "ICAEBMS Poster.jpg" },
  ...
  return (
    <section>
      <Hero 
              ...
              backgroundImage="/images/commonheroimages/commonhero7.webp"
            />
      ...
      <div className=" w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 py-16 px-4">
        {downloadList.map((item, i) => (
          <div
            ...
            className="flex flex-col justify-between items-center bg-blue-100 rounded-xl border border-gray-200 shadow hover:shadow-xl transition-shadow p-6"
          >
            <h2 className="font-semibold text-center text-lg text-blue-900 min-h-[44px] flex items-center">
              {item.title}
            </h2>
            {item.file ? (
              <a
