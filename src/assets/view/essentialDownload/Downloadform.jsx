...
const DownloadForm = ({ setPopup }) => {
  ...
  const handleSubmitDownload = async (e) => {
    e.preventDefault(); // fix: preventDefault on event, not undefined 'event'
    ...
    try {
      ...
      if (response.status === 201) {
        // Create and click anchor tag with download attribute
        const link = document.createElement("a");
        link.href = "/files/ICAEBMS%20Brochure.pdf"; // file should exist in public/files/
        link.setAttribute("download", "ICAEBMS_Brochure.pdf");
  ...
  return (
    <div className="fixed inset-0 p-3 flex justify-center items-center max-h-full min-h-screen bg-black/30 backdrop-blur-sm overflow-hidden">
      <div className="bg-white p-3 md:p-5 rounded-lg shadow-lg w-full max-w-md">
        ...
        <div className="p-3 md:p-5 flex flex-col justify-center items-center space-y-4">
          <h1 className="text-2xl text-violet-600 text-center font-bold  mt-42">
            Download ICAEBMS Brochure
          </h1>
          ...
          <button
            ...
            className="mt-4 bg-green-700 text-white px-4 py-2 rounded-full cursor-pointer text-center"
          >
            Download
          </button>
        </div>
