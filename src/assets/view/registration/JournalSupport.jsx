...
const JournalSupport = ({ selectedJournal, setSelectedJournal }) => {
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isSelected
                  ? "border-blue-600 shadow-lg shadow-blue-600/20 bg-blue-600"
                  : "border-gray-200 bg-blue-50 hover:border-blue-400 hover:shadow-md"
                }
              `}
            >
              {/* Badge */}
              ...
              <p className={`text-[10px] font-semibold uppercase tracking-widest mb-4 ${isSelected ? "text-blue-100" : "text-gray-400"}`}>
                {pkg.package}
              </p>

              {/* Pricing */}
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-blue-400 text-xs font-semibold">⚡ Exclusive Offer</span>
                <span className={`text-2xl font-extrabold ${isSelected ? "text-white" : "text-orange-500"}`}>
                  ${pkg.specialPrice.toLocaleString()}
                </span>
              </div>
              <p className={`text-xs mb-5 ${isSelected ? "text-blue-100" : "text-gray-400"}`}>
                Regular:{" "}
  ...
  <button
    ...
    className="py-2 px-4 rounded-xl text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ml-auto"
    style={{
      border: "1px solid rgba(58,74,0,0.3)",
      color: "#3a4a00",
    }}
    onMouseEnter={(e) => {
  ...
  >
    <span className="text-blue-600">👁 View Package Inclusions</span> 
  </button>
</div>

      {/* Selected summary bar */}
      {selectedJournal && (
        <div className="mt-5 flex items-center justify-between bg-blue-600 rounded-xl px-5 py-3 shadow-md shadow-blue-200">
